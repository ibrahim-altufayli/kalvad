    ...
    def get(self, request, id):
        try:
            ...
            if shopping_cart.status == CartStatus.CONFIRMED:
                return render(
                    request=request,
                    template_name="shopping_cart/already_confirmed_cart.html",
                )
            shopping_cart_ctx = {
                "id": shopping_cart.id,
            ...
            return render(
                request=request,
                template_name="shopping_cart/oppened_cart_details.html",
                context=ctx,
            )
        except ShoppingCart.DoesNotExist:
            return render(
                request=request, template_name="shopping_cart/cart_not_found.html"
            )
...
class ConfirmCartView(View):
    def post(self, request):
        ...
        errors = []
        for item in data:
            inventoryStorage = ProductUnit.objects.get(
                pk=item["product_unit_id"]
            ).inventory_product_unit_storage
            if inventoryStorage.quantity < item["quantity"]:
                error_message = "We only have {0} {1} of {2} left".format(
                    inventoryStorage.quantity,
                    inventoryStorage.product_unit.name,
                    inventoryStorage.product_unit.product.name,
                )
        ...
        for idx, item in enumerate(data):
            ...
            inventoryStorage.quantity -= shoping_cart_item.quantity
        ...
        return JsonResponse(
            {"errors": [], "message": "Cart Confirmed Thanks for choosing us!"}
        )
