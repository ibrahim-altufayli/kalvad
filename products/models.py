...
class ProductUnit(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if self.is_default:
            Product.objects.get(pk=self.product.id).product_units.exclude(
                pk=self.id
            ).update(is_default=False)
        # create new Inventory Storage Record for newly created Product Units
        if not hasattr(self, "inventory_product_unit_storage"):
            inventoryProductUnitStorage = InventoryProductUnitStorage(
                product_unit=self, quantity=0
            )
            inventoryProductUnitStorage.save()

    def __str__(self) -> str:
        return self.product.name + " | " + self.name


class InventoryProductUnitStorage(models.Model):
